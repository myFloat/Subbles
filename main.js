...
var clickOffset = [];
//Holds the vector for the offset of the mouse position from the clicked objects origin
//Keyboard
/*var keyboard = {
...
class Subble {
    ...
    gridAlign(TRAVELERS) {
        ...
        if (this.parents.length > 0) {
            ...
            if (max(abs(this.gridPos[0]), abs(this.gridPos[1])) > Sbls.parentMaxGap) {
                //If any coordinate is beyond limit
    ...
    forOffspring(FUNCTION) {
        //Do for all children, grandchildren, a.s.f...
        for (const obj1 of this.children) {
            FUNCTION(obj1, this);
...
var Sbls = {
    ...
    generationGap: 1 / 2,
    //Size proportion from each subble to its child
    parentMaxGap: 9999,
    //Max allowed distance to parents in local coordinates
    input: null,
    //Menu
    menu: null,
    menuPos: [0, 0],
